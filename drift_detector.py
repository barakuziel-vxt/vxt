...
def detect_entity_drift(**kwargs) -> Dict:
    ...
    This function is called by the worker with:
    - entity_id: The entity being analyzed
    - event_id: The event ID associated with this analysis
    - event_criteria: List of criteria dicts with attribute info
    - telemetry_data: Current telemetry data (15-min window)
    - connection: Database connection for fetching extended telemetry
    - triggered_at: Current timestamp
    - analysis_window_min: Analysis window in minutes (15)
    - event_params: Event parameters (LookbackMinutes, BaselineDays, SensitivityThreshold, etc.)
    ...
    try:
        ...
        # Get event configuration from database to retrieve drift parameters
        cursor = connection.cursor()
        
        # Query the Event table for the specific event's parameters
        event_query = """
        SELECT 
            LookbackMinutes,
            BaselineDays,
        ...
        lookback_minutes = event_row[0] or 1440  # Default 24 hours
        baseline_days = event_row[1] or 7        # Default 7 days
        sensitivity_threshold = float(event_row[2] or 1.8)  # Default 1.8 sigma
        min_samples = int(event_row[3] or 5)     # Default 5 samples
        anomaly_direction = (event_row[4] or 'BOTH').upper()  # Default BOTH
        
        # Convert anomaly direction to drift detector format
        ...
        # Get the attribute to analyze from event criteria (use first attribute)
        ...
        # Fetch current telemetry (last N minutes)
        cursor = connection.cursor()
        ...
        # Fetch baseline telemetry (historical data: 7 days ago to now, minus the current lookback window)
        time_cutoff_baseline_end = time_cutoff_current  # End is when current window starts
        ...
        # Prepare parameters for detect_metric_drift
        drift_params = {
            'sensitivity_threshold': sensitivity_threshold,
            'direction': direction,
            'min_current_points': min_samples,
            'min_baseline_points': max(5, min_samples),  # At least 5 baseline points
            'lookback_hours': lookback_minutes / 60.0,
            'baseline_days': baseline_days
        }
        
        # Call the core drift detection function
        ...
        # Prepare details for EventLogDetails insertion
        # Include entityTypeAttributeId, scoreContribution, and withinRange for database logging
        detail_record = {
            'entityTypeAttributeId': attribute_id,
            'scoreContribution': int(drift_result['score']),  # Use the drift score
            'withinRange': 'true' if drift_result['is_alert'] else 'false',  # 'true' if drift detected (outside baseline)
        ...
        # Prepare analysis metadata for EventLog (for AI function context)
        # This captures the statistical findings that led to the event detection
        analysis_metadata = {
            'functionType': 'PYTHON',
            ...
            'baselineAnalysis': {
                'avgValue': round(drift_result['baseline_avg'], 2),
                'sampleCount': len(baseline_telemetry),
                'dateRange': f"{(current_time - timedelta(days=baseline_days)).isoformat()} to {time_cutoff_current.isoformat()}"
            },
            'currentAnalysis': {
                'avgValue': round(drift_result['current_avg'], 2),
                'sampleCount': len(current_telemetry),
                'dateRange': f"{time_cutoff_current.isoformat()} to {current_time.isoformat()}"
            },
            'detectionMetadata': {
                'method': 'z-score',
                'threshold': sensitivity_threshold,
                'z_score': round(drift_result['z_score'], 3),
                'drift_percentage': round(drift_result['drift_percentage'], 2),
                'sensitivity': 'high' if sensitivity_threshold < 1.5 else 'medium' if sensitivity_threshold < 2.0 else 'low'
            }
        }
        ...
        return {
            ...
            'analysisMetadata': analysis_metadata  # For storing in EventLog.analysisMetadata
        }
