...
BEGIN
    ...
    -- 5. Body calories burned (41981-2)
    IF NOT EXISTS (SELECT 1 FROM ProviderEvent WHERE providerId = @JunctionProviderId AND protocolAttributeCode = '41981-2')
    BEGIN
        INSERT INTO ProviderEvent (providerId, providerEventType, providerEventDescription, providerNamespace, providerEventName, providerVersion, protocolAttributeCode, payloadSchema, requiredFields)
        VALUES (@JunctionProviderId, 'activity.calories_burned.update', 'Total calories burned including basal metabolic rate', 'activity', 'calories_burned', '1.0', '41981-2',
            N'{"type":"object","properties":{"caloriesBurned":{"type":"number","description":"Calories burned","minimum":0},"unit":{"type":"string","enum":["kcal"]},"timestamp":{"type":"string","format":"date-time"},"source":{"type":"string"}}}',
            N'["caloriesBurned","timestamp"]');
        PRINT 'Inserted: Body calories burned (41981-2)';
    END
    
    -- 6. Percentage of body fat (41982-0)
    ...
    BEGIN
        ...
        SELECT 
            ...
            CASE 
                ...
                WHEN pe.protocolAttributeCode = '29463-7' THEN 'Body weight (kg)'
                WHEN pe.protocolAttributeCode = '41981-2' THEN 'Body calories burned'
                WHEN pe.protocolAttributeCode = '41982-0' THEN 'Body fat percentage'
            ...
            CASE 
                ...
                WHEN pe.protocolAttributeCode = '55430-3' THEN 'km'
                WHEN pe.protocolAttributeCode = '29463-7' THEN 'kg'
                WHEN pe.protocolAttributeCode = '41981-2' THEN 'kcal'
        ...
        WHERE pe.providerId = @JunctionProviderId
          AND pe.protocolAttributeCode IN ('55423-8', '93832-4', '55430-3', '29463-7', '41981-2', '41982-0')
          AND NOT EXISTS (
    ...
    COMMIT;
    PRINT '✅ Migration complete: Added 6 missing LOINC codes';
END
